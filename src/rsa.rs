...
pub fn rsa_encrypt<'py>(py: Python<'py>, data: &[u8], fingerprint: i64) -> PyResult<Bound<'py, PyBytes>> {
    ...
    let bytes = PyBytes::new_bound_with(py, 256, |out| {
        py.allow_threads(|| {
            ...
            let encrypted_int = Integer::from(encrypted);

            // Convert to 256-byte big-endian representation directly into output buffer
            out.fill(0);
            let written = encrypted_int.write_digits(out, Order::Msf);
            
            // If the written digits are less than 256, they are already padded with 0s at the end
            // because write_digits writes from the start. Wait, Msf means Most Significant First.
            // If we want it to be right-aligned (padded with leading zeros), we need to handle that.
            if written < 256 {
                out.copy_within(0..written, 256 - written);
                out[..256 - written].fill(0);
            }
        });
        Ok(())
    })?;
